...
param(
    [string]$OutputPath = "D:\AICode\openclaw\logs\security-reports"
)
...
function Test-OpenClawConfig {
    ...
    if (Test-Path $configPath) {
        ...
        # Check hash
        $baselinePath = "D:\AICode\openclaw\.openclaw\.config-baseline.sha256"
...
function Test-CredentialLeak {
    $leaked = @()
    $memoryPath = "D:\AICode\openclaw\.openclaw\workspace\memory"
