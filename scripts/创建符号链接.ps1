...
if (Test-Path $linkPath) {
    ...
    if ($item.LinkType -eq "SymbolicLink") {
        Write-Host "⚠️ 符号链接已存在" -ForegroundColor Yellow
        ...
        if ($item.Target -eq $targetPath) {
            ...
            Write-Host "  ⚠️ 指向不同路径，需要先删除" -ForegroundColor Yellow
            Write-Host ""
...
Write-Host "  D:\openclaw\scripts\xxx.ps1" -ForegroundColor Green
Write-Host "  D:\openclaw\.openclaw\openclaw.json" -ForegroundColor Green
Write-Host ""
Write-Host "这和以下路径完全等价：" -ForegroundColor Yellow
Write-Host "  D:\openclaw\scripts\xxx.ps1" -ForegroundColor Cyan
Write-Host "  D:\openclaw\.openclaw\openclaw.json" -ForegroundColor Cyan
Write-Host ""
Write-Host "按任意键退出..." -ForegroundColor Yellow
$null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
