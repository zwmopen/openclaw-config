...
if ($state.status -eq "running") {
    ...
    if ($files.Count -eq 0) {
        ...
        if ($idx -lt $dirs.Count - 1) {
            ...
            $state.scanned = 0
            Write-Output "切换到目录: $($state.currentDir)"
        } else {
        ...
        foreach ($file in $files) {
            ...
            if ($content) {
                if ($content -match "(本质|规律|原则|核心|底层逻辑|第一性原理|方法论|关键|重要)") {
                    $concepts++
                }
                if ($content -match "(所谓|真正的|重要的|关键的|记住|明白|发现|其实|不是.*而是|因为.*所以|只有.*才|没有.*就)") {
                    $quotes++
                }
                if ($content -match "(案例|例子|故事|经历|实践|实操|落地|应用)") {
                    $cases++
                }
            }
        }
        ...
        Write-Output "扫描了 $($files.Count) 个文件"
        Write-Output "概念: +$concepts (总: $($state.concepts.completed)/$($state.concepts.target))"
        Write-Output "金句: +$quotes (总: $($state.quotes.completed)/$($state.quotes.target))"
        Write-Output "案例: +$cases (总: $($state.cases.completed)/$($state.cases.target))"
        Write-Output "总进度: $($state.scanned)/$($state.total) ($progress%)"
    }
    
    $state | ConvertTo-Json -Depth 10 | Set-Content $stateFile -Encoding UTF8
} else {
    Write-Output "状态: $($state.status)"
}
