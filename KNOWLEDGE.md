...
### Windows批处理中文路径编码问题（2026-03-06）

**问题**：批处理文件（.bat）在中文路径下执行PowerShell命令会失败。
...
- 批处理文件默认使用GBK编码
- 路径中有中文字符时，编码不匹配
- PowerShell命令被截断成单个字符

**解决方案**：

| 方案 | 说明 | 推荐 |
|------|------|------|
| A. 放到英文路径 | 如 `C:\Temp\script.bat` | ⭐⭐⭐ 最简单 |
| B. 切换UTF-8编码 | 批处理开头加 `chcp 65001` | ⭐⭐ 有时仍有问题 |
| C. 直接用PowerShell | `.ps1` 文件对中文支持更好 | ⭐⭐⭐ 推荐 |

**最佳实践**：
- 需要管理员权限的脚本 → 放到 `C:\Temp\`
- 日常脚本 → 直接用 `.ps1`
- 路径有中文 → 避免用 `.bat`
...
```powershell
# 错误（中文路径）
D:\AI编程\openclaw\scripts\run.bat
